...
**받는 일의 나머지는 `BOT.md`에 있다** — 순회(`../check-forks.sh`)와, 받은 뒤에 왜 검사를
돌려야 하는가. **포크도 알아야 하는 것이라 이 파일에 두지 않는다**: 이 파일은 포크가 자기
것으로 갈아엎으므로, 여기 적은 것은 포크에 닿지 않을 뿐 아니라 **merge마다 부딪힌다**
(2026-09-24에 한 번 그랬다).
